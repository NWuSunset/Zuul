...
bool goRoom(Command* command, Rooms* & currentRoom, vector<Items*>& inventory);
...
int main() {
    ...
    while (!gameEnd) {
        ...
        delete command;
        //Since there can be a lot of commands, delete the old command pointer after the command is executed to free up memory
    }
    return 0;
}
...
bool goRoom(Command* command, Rooms* & currentRoom, vector<Items*>& inventory) {
    //Goes to the room in the direction entered (north, south, east, west)
    char direction[20];
    char key[] = "StudentStoreKey";
    ...
    if (nextRoom == nullptr) {
        //if there are no exits this will be null as set by Rooms.cpp
        cout << "There is no exit that way" << endl;
    } else if (strcasecmp(nextRoom->getDescription(), "You are now in the student store") == 0 && !hasItem(
        key, inventory)) {
        cout << "Hmm. The student store is locked, I wonder if there is a key lying around somewhere" << endl;
        return false;
    } else if (strcasecmp(nextRoom->getDescription(), "You are in a secret room") == 0 && !
        hasItem(lockpick, inventory)) {
...
bool getItem(Command* command, vector<Items*>& inventory, Rooms* & currentRoom) {
    ...
    if (newItem == nullptr) {
        ...
        if (strcasecmp(item, "Snacks") == 0) {
            //win condition check
            if (!hasItem(pass, inventory)) {
                //Check if player has snack pass
                cout <<
                    "You need a snack pass to get the snacks here. I wonder what place would have a snack pass, probably somewhere with a lot of snacks"
                    << endl;
                return false;
            }
            //Well technically we never need to add the snacks to the inventory...
        ...
        if (strcasecmp(item, "StudentStoreKey") == 0) {
            cout << "Now I can get into the student store!" << endl;
        } else if (strcasecmp(item, "SnackPass") == 0) {
            cout << "Now I can get snacks in the secret room!" << endl;
        } else if (strcasecmp(item, "LockPick") == 0) {
...
bool dropItem(Command* command, vector<Items*>& inventory, Rooms* & currentRoom) {
    ...
    for (vector<Items*>::iterator it = inventory.begin(); it != inventory.end(); ++it) {
        //using iterator here in order to use an index to delete stuff
...
void printHelp(Parser* parser) {
    ...
    if (strcasecmp(input, "yes") == 0) {
        ...
        cout << "You declined the hint" << endl;
    }
}

void listRooms() {
    char roomList[17][40] = {
        "Outside", "Theater", "Student Store", "Bathroom one", "Lab", "Office", "Computer Lab", "Physics room",
